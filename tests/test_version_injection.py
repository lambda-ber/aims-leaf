...
SCHEMA_PATH = Path(__file__).parent.parent / "src" / \
    "aims_leaf_schema" / "schema" / "aims_leaf_ber_schema.yaml"
...
def _repo_has_version_tags() -> bool:
    """Return True when the repository has at least one git tag for dynamic versioning."""
...
def test_build_injects_version():
    ...
    with zipfile.ZipFile(wheel_path, "r") as zf:
        ...
        ) if n.endswith("aims_leaf_ber_schema.yaml")]
        assert len(
    ...
    # In a brand-new repository with no tags, uv-dynamic-versioning falls back to 0.0.0.
    if _repo_has_version_tags():

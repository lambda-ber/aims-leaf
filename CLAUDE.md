...
aims-leaf is a LinkML schema project for representing structural biological imaging data and experiments. It provides a unified data model for cryo-EM, X-ray crystallography, SAXS/WAXS, and SANS techniques with rich biological context annotations.
...
### Schema Structure
The main schema definition is located at `src/aims_leaf_schema/schema/aims_leaf_ber_schema.yaml`. This LinkML schema uses a **relational design** with flat entity collections and explicit association tables for M:N relationships.
...
- Python dataclasses (`aims_leaf_schema.py`)
...
make gen-project
# or directly:
uv run gen-project src/aims_leaf_schema/schema/aims_leaf_ber_schema.yaml -d assets
```
...
make test
```
This currently regenerates schema artifacts. Example validation has been removed from the default test path during the ongoing schema refactor.

### Schema Validation
Validate the schema against LinkML metamodel:
```bash
uv run linkml-lint src/aims_leaf_schema/schema/aims_leaf_ber_schema.yaml
```

### Data Validation
Validate data files against the schema:
```bash
uv run linkml-validate -s src/aims_leaf_schema/schema/aims_leaf_ber_schema.yaml <data_file>
```
...
- `src/aims_leaf_schema/schema/aims_leaf_ber_schema.yaml` - Main LinkML schema definition

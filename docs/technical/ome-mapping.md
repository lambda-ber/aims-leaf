# Technical Mapping: OME to aims-leaf

## Overview

This document provides detailed technical mappings between OME (Open Microscopy Environment) data model elements and corresponding aims-leaf schema components, with code examples and implementation guidance.

## Core Entity Mappings

### Image Entities

#### OME Image → aims-leaf Image Classes
...
#### OME Instrument → aims-leaf Instrument
...
**aims-leaf Equivalent (extending for compatibility):**
```yaml
instruments:
...
#### OME Experiment → aims-leaf Study/ExperimentRun
...
#### OME 5D Data (XYZCT) → aims-leaf Multi-Frame Images
...
#### OME ROI → aims-leaf Annotation Extension
...
### OME-TIFF ↔ aims-leaf Integration

```python
# Pseudo-code for conversion
from bioformats import load_image_metadata
from aims-leaf import FluorescenceImage

def ome_to_aims-leaf(ome_tiff_path):
    """Convert OME-TIFF to aims-leaf data model"""
    
    # Read OME-XML from TIFF header
    ome_xml = load_image_metadata(ome_tiff_path)
    
    # Create aims-leaf image
...
def create_bioformats_reader():
    """Create Bio-Formats reader for aims-leaf images"""
    
    class aims-leafReader:
        def read_fluorescence_image(self, aims-leaf_image):
            # Convert to OME model
            ome_image = {
                'ID': aims-leaf_image.id,
                'Name': aims-leaf_image.file_name,
                'AcquisitionDate': aims-leaf_image.acquisition_date,
                'Pixels': {
                    'SizeX': aims-leaf_image.dimensions_x,
                    'SizeY': aims-leaf_image.dimensions_y,
                    'Channels': [{
                        'ExcitationWavelength': aims-leaf_image.excitation_wavelength,
                        'EmissionWavelength': aims-leaf_image.emission_wavelength,
                        'Name': aims-leaf_image.fluorophore
                    }]
                }
            }
            return ome_image
...
| OME Path | aims-leaf Path | Type | Notes |
|----------|----------------|------|-------|
...
aims-leaf extends beyond OME for structural biology:

| aims-leaf Path | Purpose | No OME Equivalent |
|----------------|---------|-------------------|
...
### 1. Creating OME-Compatible aims-leaf Data

```yaml
# Template for OME-compatible fluorescence microscopy in aims-leaf
studies:
...
```python
# Upload aims-leaf images to OMERO
def upload_to_omero(aims-leaf_study):
    import omero.gateway
    
    conn = omero.gateway.BlitzGateway(...)
    
    for image in aims-leaf_study.images:
        if isinstance(image, FluorescenceImage):
            ...
            # Add aims-leaf-specific metadata as key-value pairs
            map_ann = omero.gateway.MapAnnotationWrapper()
            map_ann.setValue([
                ["aims-leaf:study_id", aims-leaf_study.id],
                ["aims-leaf:sample_code", image.sample.sample_code],
                ["aims-leaf:technique", "fluorescence"]
            ])
...
def validate_ome_compatibility(image):
    """Check if aims-leaf image can be exported to OME"""
...
```yaml
# OME-style plate/well structure in aims-leaf
studies:
  - id: study:hcs-001
...
This technical mapping demonstrates that aims-leaf can achieve strong interoperability with OME while extending the model for structural biology needs. Key implementation priorities include:
...
The alignment enables researchers to use familiar OME tools while benefiting from aims-leaf's structural biology capabilities.
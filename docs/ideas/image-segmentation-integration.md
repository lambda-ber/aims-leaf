...
## Integration with AIMS-LEAF

### Current Schema Coverage

The existing aims-leaf already supports key aspects of segmentation workflows:
...
```python
# Conceptual data pipeline using Delta Lake + aims-leaf
...
def ingest_raw_data(tiff_path, schema_metadata_path):
    """Bronze layer: raw data + aims-leaf metadata"""
    ...
    (spark.createDataFrame([metadata])
        ...
        .format("delta")
        .mode("append")
        .save("s3://lakehouse/bronze/metadata/aims-leaf/"))
...
def run_segmentation_pipeline(experiment_code):
    ...
    metadata_df = (spark.read.format("delta")
        .load("s3://lakehouse/bronze/metadata/aims-leaf/")
    ...
    # Create workflow metadata for aims-leaf
    workflow = {
...
def analyze_catalyst_performance():
    ...
    SELECT
        ...
        f.mean_pore_size,
        f.interfacial_area
    FROM delta.`s3://lakehouse/bronze/metadata/aims-leaf/` s
...
Building on Zenesis's evaluation dashboard (Figure 8 in the paper), integrate with aims-leaf for comprehensive monitoring:
...
# Query metadata from aims-leaf + results
df = con.execute("""
    SELECT
        w.workflow_code,
    ...
    JOIN read_json_auto('s3://lakehouse/bronze/metadata/aims-leaf/*.json') schema

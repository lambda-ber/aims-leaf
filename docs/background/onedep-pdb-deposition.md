...
This document describes the practical workflow for depositing structures to the Protein Data Bank (PDB) and how aims-leaf relates to the deposition process.
...
Researchers manually transcribe from multiple sources into OneDep forms - an error-prone and tedious process. This is exactly the gap aims-leaf aims to fill.
...
This maps to aims-leaf's `ExperimentRun` class and the X-ray specific fields.
...
The following table shows how each PDB header record maps to specific classes and slots in aims-leaf.
...
1. Generate metadata summary from aims-leaf records
...
OneDep performs extensive validation before accepting a deposition. Understanding these checks helps ensure aims-leaf captures the right metadata.
...
These are frequent problems researchers encounter during deposition, and how aims-leaf helps prevent them:
...
Conceptual example of generating OneDep-ready metadata from aims-leaf:

```python
def generate_onedep_metadata(study: Study) -> dict:
    """Extract OneDep form data from aims-leaf Study."""

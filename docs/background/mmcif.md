# aims-leaf and mmCIF Alignment Analysis
...
This document analyzes the alignment between the aims-leaf schema and mmCIF (macromolecular Crystallographic Information File), the standard format for structural biology data. We examine compatibility, complementary features, and integration strategies between these two important data standards.
...
| mmCIF Category | aims-leaf Equivalent | Alignment Notes |
|----------------|---------------------|-----------------|
...
| **_atom_site** | Not directly modeled | aims-leaf doesn't store atomic coordinates |
...
**aims-leaf**: Workflow-level tracking
...
**aims-leaf**: Complete experimental timeline
...
**aims-leaf**: Flexible semantic model
...
### aims-leaf → mmCIF Export

```python
# Conceptual mapping for structure deposition
def aims-leaf_to_mmcif(study):
    mmcif_data = {
        '_entry.id': study.id,
...
### mmCIF → aims-leaf Import

```yaml
# aims-leaf representation of mmCIF data
Study:
...
aims-leaf concepts map well to these extensions:
...
```
aims-leaf: Track samples, instruments, experimental runs
         ↓
...
### 2. Processing Phase
```
aims-leaf: Document workflows, software, parameters
         ↓
...
```
aims-leaf + mmCIF: Combine metadata and coordinates
         ↓
...
aims-leaf: Maintain complete experimental record
```
...
Needed utilities:
- `aims-leaf2mmcif`: Export aims-leaf metadata to mmCIF categories
- `mmcif2aims-leaf`: Import PDB entries as aims-leaf studies
...
aims-leaf and mmCIF serve **complementary roles** in the structural biology data ecosystem:

- **mmCIF** is the definitive standard for atomic structure representation and PDB deposition
- **aims-leaf** provides comprehensive workflow and multi-modal experiment tracking

The optimal strategy involves:
1. Using aims-leaf for experiment management and data integration
...
This complementary relationship ensures that both the journey (aims-leaf) and destination (mmCIF) of structural biology research are properly documented and preserved.
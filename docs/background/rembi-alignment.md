# REMBI Alignment: Integrating Biological Imaging Metadata Standards with aims-leaf

## Executive Summary

REMBI (Recommended Metadata for Biological Images) represents a community-driven standard for biological imaging metadata that shares significant overlap with aims-leaf's goals for structural biology data management. Published in Nature Methods in 2021, REMBI provides guidelines for describing imaging experiments across light microscopy, electron microscopy, and X-ray microscopy—making it highly relevant to aims-leaf's multi-modal approach. This document analyzes the alignment between REMBI and aims-leaf, identifying opportunities for integration and mutual reinforcement of metadata standards in structural biology.
...
## 3. Alignment with aims-leaf

### 3.1 Structural Mapping

aims-leaf and REMBI share substantial conceptual overlap, with complementary strengths:

| REMBI Component | aims-leaf Equivalent | Alignment Level | Notes |
|-----------------|---------------------|-----------------|-------|
...
| Image Acquisition | Instrument + ExperimentRun | **High** | aims-leaf separates instrument from run |
| Image Data | Image (2D/3D) + DataFile | **High** | aims-leaf has specialized image classes |
| Image Correlation | Cross-modal relationships | **Medium** | aims-leaf handles via Study relationships |
| Analyzed Data | WorkflowRun + DataFile | **High** | Processing and derived data |

### 3.2 Complementary Strengths

#### REMBI Strengths for aims-leaf
...
Integration_Strategy:
  metadata_harmonization:
    - Map REMBI fields to aims-leaf schema
    - Extend aims-leaf with REMBI-specific attributes
  ...
  tool_ecosystem:
    - Support REMBI export from aims-leaf
...
REMBI provides the metadata framework for these techniques that aims-leaf can leverage.
...
Create explicit mappings between REMBI and aims-leaf:

```python
class REMBIaims-leafMapper:
    """Bidirectional mapper between REMBI and aims-leaf schemas"""
    
    def rembi_to_aims-leaf(self, rembi_data):
        aims-leaf_study = {
        ...
        if 'image_correlation' in rembi_data:
            aims-leaf_study['cross_modal_registration'] = \
                self.map_correlation(rembi_data['image_correlation'])
        
        return aims-leaf_study
    
    def aims-leaf_to_rembi(self, aims-leaf_data):
...
Extend aims-leaf to incorporate REMBI-specific fields:

```yaml
# Addition to aims-leaf schema
ImageCorrelation:
  is_a: AttributeGroup
...
class CrossStandardValidator:
    """Validate data against both REMBI and aims-leaf"""
    
    def validate(self, data):
        results = {
            'aims-leaf_valid': self.validate_aims-leaf(data),
        ...
        if results['aims-leaf_valid'] and not results['rembi_valid']:
            results['warnings'].append(
                "Data valid for aims-leaf but missing REMBI requirements"
            )
...
Future_Standard:
  ...
  foundation: "LinkML"
  components:
    - structural_biology: "From aims-leaf"
    - light_microscopy: "From REMBI"
    - image_correlation: "From REMBI"
    - workflow_provenance: "From aims-leaf"
    - ai_readiness: "From both"
  
  governance:
    - committee: "Joint REMBI-aims-leaf working group"
...
Study:
  ...
  # aims-leaf-style experiment runs
  experiments:
...
Study:
  ...
  cross_scale_correlation:
    ...
    metadata_standard: "REMBI+aims-leaf"
```
...
- Extend aims-leaf schema with REMBI fields
...
The alignment between REMBI and aims-leaf represents a critical opportunity to bridge the imaging and structural biology communities. With REMBI's broad adoption in biological imaging and aims-leaf's comprehensive structural biology support, integration of these standards will enable unprecedented multi-scale, multi-modal studies.
...
- aims-leaf gains established imaging metadata standards and community adoption
...
2. **Short-term**: Implement REMBI support in aims-leaf tools
3. **Medium-term**: Joint workshops and training materials
4. **Long-term**: Co-develop next-generation unified standards

The convergence of REMBI and aims-leaf will accelerate scientific discovery by enabling researchers to seamlessly navigate from cellular contexts to atomic details, ultimately advancing our understanding of biological systems across all scales.

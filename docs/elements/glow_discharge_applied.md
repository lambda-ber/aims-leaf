

# Slot: glow_discharge_applied 


_Whether glow discharge treatment was applied_





URI: [aimsleaf:glow_discharge_applied](https://w3id.org/aims-leaf/glow_discharge_applied)
Alias: glow_discharge_applied

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [CryoEMPreparation](CryoEMPreparation.md) | Cryo-EM specific sample preparation |  no  |






## Properties

* Range: [Boolean](Boolean.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/aims-leaf/




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | aimsleaf:glow_discharge_applied |
| native | aimsleaf:glow_discharge_applied |




## LinkML Source

<details>
```yaml
name: glow_discharge_applied
description: Whether glow discharge treatment was applied
from_schema: https://w3id.org/aims-leaf/
rank: 1000
alias: glow_discharge_applied
owner: CryoEMPreparation
domain_of:
- CryoEMPreparation
range: boolean

```
</details>
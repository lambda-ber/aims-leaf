

# Slot: exposure_time_per_frame 


_Exposure time per frame in milliseconds_





URI: [aimsleaf:exposure_time_per_frame](https://w3id.org/aims-leaf/exposure_time_per_frame)
Alias: exposure_time_per_frame

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ExperimentRun](ExperimentRun.md) | An experimental data collection session |  no  |






## Properties

* Range: [QuantityValue](QuantityValue.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/aims-leaf/




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | aimsleaf:exposure_time_per_frame |
| native | aimsleaf:exposure_time_per_frame |




## LinkML Source

<details>
```yaml
name: exposure_time_per_frame
description: Exposure time per frame in milliseconds
from_schema: https://w3id.org/aims-leaf/
rank: 1000
alias: exposure_time_per_frame
owner: ExperimentRun
domain_of:
- ExperimentRun
range: QuantityValue
inlined: true

```
</details>
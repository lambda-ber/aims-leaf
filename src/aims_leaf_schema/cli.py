...
from aims_leaf_schema.loaders import (
    BatchLoader,
    EMSLLoader,
    PDBLoader,
    ResponseCache,
...
def etl_sasbdb(
    ...
    Examples:
        ...
        aims-leaf etl sasbdb --entry SASDA52

        aims-leaf etl sasbdb --entry SASDA52 --output data.yaml

        aims-leaf etl sasbdb --entry SASDA52 --format json --cache
    """
...
def etl_simplescattering(
    ...
    Examples:
        ...
        aims-leaf etl simplescattering --dataset xsbhevph

        aims-leaf etl simplescattering --dataset xsbhevph --output data.yaml

        aims-leaf etl simplescattering --dataset xsbhevph --format json --cache
    """
    # Set up loader with optional cache
    response_cache = ResponseCache(
        cache_dir=cache_dir or Path(".cache"),
...
def etl_pdb(
    ...
    Examples:
        ...
        aims-leaf etl pdb --entry 1HHO

        aims-leaf etl pdb --entry 7S4S --output data.yaml

        aims-leaf etl pdb --entry 1HHO --format json --cache
    """
    # Set up loader with optional cache
    response_cache = ResponseCache(
        cache_dir=cache_dir or Path(".cache"),
...
def etl_emsl(
    ...
    Examples:
        ...
        aims-leaf etl emsl --sample apo

        aims-leaf etl emsl --sample apo --transaction-id 3736677

        aims-leaf etl emsl --sample apo --format json --cache
    """
    response_cache = ResponseCache(
        cache_dir=cache_dir or Path(".cache"),
        enabled=cache,
...
def etl_list(
    ...
    Examples:
        ...
        aims-leaf etl list sasbdb

        aims-leaf etl list sasbdb --type protein --limit 10

        aims-leaf etl list simplescattering --limit 5

        aims-leaf etl list pdb --method X-RAY --limit 10

        aims-leaf etl list emsl --sample apo --limit 10
    """
...
def etl_dump_pdb(
    ...
    Examples:
        ...
        aims-leaf etl dump-pdb --output-dir ./pdb_dump

        # Load only X-ray structures
        aims-leaf etl dump-pdb --output-dir ./pdb_xray --method X-RAY

        # Load first 100 entries for testing
        aims-leaf etl dump-pdb --output-dir ./pdb_test --limit 100

        # Resume after interruption
        aims-leaf etl dump-pdb --output-dir ./pdb_dump

        # Retry failed entries
        aims-leaf etl dump-pdb --output-dir ./pdb_dump --retry-failed
    """
...
def etl_dump_sasbdb(
    ...
    Examples:
        ...
        aims-leaf etl dump-sasbdb --output-dir ./sasbdb_dump

        # Load only protein entries
        aims-leaf etl dump-sasbdb --output-dir ./sasbdb_protein --type protein

        # Load first 50 entries for testing
        aims-leaf etl dump-sasbdb --output-dir ./sasbdb_test --limit 50

        # Resume after interruption
        aims-leaf etl dump-sasbdb --output-dir ./sasbdb_dump

        # Retry failed entries
        aims-leaf etl dump-sasbdb --output-dir ./sasbdb_dump --retry-failed
    """
    output_dir.mkdir(parents=True, exist_ok=True)
...
def version() -> None:
    ...
    try:
        ...
        v = get_version("aims-leaf")
    except Exception:
        v = "unknown"

    typer.echo(f"aims-leaf {v}")

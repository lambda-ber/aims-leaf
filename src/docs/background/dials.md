# aims-leaf and DIALS Alignment Analysis
...
This document analyzes the alignment between the aims-leaf schema and DIALS (Diffraction Integration for Advanced Light Sources), examining how aims-leaf can integrate with and complement this modern crystallography data processing framework.
...
## aims-leaf-DIALS Alignment

### Structural Mapping

| DIALS Concept | aims-leaf Equivalent | Alignment Notes |
|---------------|---------------------|-----------------|
...
### aims-leaf → DIALS Export

```python
def export_to_dials(experiment_run):
    """Generate DIALS experiment from aims-leaf"""
    from dxtbx.model import Beam, Detector, Goniometer
    
    # Create DIALS models from aims-leaf
    beam = Beam()
    beam.set_wavelength(experiment_run.wavelength)
    
    detector = Detector()
    # Map aims-leaf instrument to detector parameters
...
### DIALS → aims-leaf Import

```python
def import_dials_results(expt_file, refl_file):
    """Import DIALS processing into aims-leaf"""
    from dxtbx.model.experiment_list import ExperimentList
...
Both DIALS and aims-leaf handle serial data:

```yaml
# aims-leaf serial crystallography
ExperimentRun:
...
| Feature | DIALS | XDS | CrystFEL | aims-leaf Role |
|---------|-------|-----|----------|----------------|
...
aims-leaf can track processing with any package:
...
### 1. Data Collection
```
aims-leaf: Track sample, crystal preparation, mounting
         ↓
Collect diffraction data at beamline
         ↓
Store metadata in aims-leaf
```
...
DIALS: Process diffraction data
         ↓
Track each step in aims-leaf WorkflowRun
         ↓
...
External tools: Phasing, model building
         ↓
Track in aims-leaf WorkflowRun
         ↓
...
### 4. Deposition
```
Generate mmCIF from DIALS + aims-leaf metadata
         ↓
Deposit to PDB
         ↓
Update aims-leaf with PDB ID
```
...
1. **Track Everything**: Use aims-leaf for complete experimental record
...
4. **Pipeline Automation**: Script DIALS commands with aims-leaf tracking
...
1. **Direct Integration**
   - DIALS plugin for aims-leaf export
...
aims-leaf and DIALS form a **complementary ecosystem** for crystallography data management:
...
- **aims-leaf** adds comprehensive experimental context through:
...
2. Tracking complete workflows in aims-leaf
3. Maintaining processing parameter history
4. Enabling reproducibility through comprehensive documentation

This integration ensures both the processing pipeline (DIALS) and the experimental context (aims-leaf) are properly captured, enabling reproducible crystallographic research and facilitating method development in structural biology.